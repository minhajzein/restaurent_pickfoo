...
const LocationPicker = dynamic(() => import('@/components/Map/LocationPicker'), {
  ssr: false,
  loading: () => <div className="h-[300px] w-full bg-white/5 animate-pulse rounded-2xl border border-white/10 flex items-center justify-center text-white/20">Loading Map...</div>
});
...
export default function OwnerRestaurantsPage() {
  ...
  const { useMyRestaurants, createRestaurant, submitForVerification, updateRestaurant, deleteRestaurant } = useRestaurants();
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, type: string, folder: string = 'restaurants') => {
  ...
  const handleEdit = (restaurant: Restaurant) => {
    ...
    // Docs
    setValue('legalDocs.fssaiLicenseNumber', restaurant.legalDocs.fssaiLicenseNumber);
    setValue('legalDocs.panNumber', restaurant.legalDocs.panNumber);
    setValue('legalDocs.gstNumber', restaurant.legalDocs.gstNumber);
    setValue('legalDocs.tradeLicenseNumber', restaurant.legalDocs.tradeLicenseNumber);
    ...
    if (restaurant.legalDocs.fssaiCertificateUrl) {
      ...
      setValue('legalDocs.fssaiCertificateUrl', restaurant.legalDocs.fssaiCertificateUrl);
    }
    if (restaurant.legalDocs.gstCertificateUrl) {
      docsToAdd['gstCertificate'] = restaurant.legalDocs.gstCertificateUrl;
      setValue('legalDocs.gstCertificateUrl', restaurant.legalDocs.gstCertificateUrl);
    }
    if (restaurant.legalDocs.tradeLicenseUrl) {
      docsToAdd['tradeLicense'] = restaurant.legalDocs.tradeLicenseUrl;
      setValue('legalDocs.tradeLicenseUrl', restaurant.legalDocs.tradeLicenseUrl);
    }
    if (restaurant.legalDocs.healthCertificateUrl) {
      docsToAdd['healthCertificate'] = restaurant.legalDocs.healthCertificateUrl;
      setValue('legalDocs.healthCertificateUrl', restaurant.legalDocs.healthCertificateUrl);
    }
  ...
  const handleToggleOpen = async (restaurant: Restaurant, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      ...
      toast.success(restaurant.isOpen ? 'Restaurant closed for orders' : 'Restaurant is now open');
    } catch (error) {
      if (axios.isAxiosError(error)) {
  ...
  const handleCancel = async () => {
    try {
      ...
      if (!editingId) {
        ...
        if (logoUrl) {
          await deleteFile(logoUrl).catch(err => console.error('Failed to delete logo:', err));
        }
        ...
        if (uploadedDocs.length > 0) {
          await Promise.all(
            uploadedDocs.map(url => 
              deleteFile(url).catch(err => console.error('Failed to delete doc:', err))
            )
          );
        }
      }
            ...
            <div key={restaurant._id} className="bg-white/5 border border-white/10 rounded-[2.5rem] overflow-hidden group hover:border-[#98E32F]/30 transition-all">
              <div className="relative h-48">
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                  <span className={`
                    ...
                    ${restaurant.status === 'active' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 
                      restaurant.status === 'pending' ? 'bg-orange-500/20 text-orange-400 border-orange-500/30' : 
                  ...
                  {restaurant.status === 'active' && (
                    <span className={`
                      ...
                      ${restaurant.isOpen 
                        ...
                        : 'bg-red-500/20 text-red-400 border-red-500/30'}
                    `}>
                      <div className={`w-1.5 h-1.5 rounded-full ${restaurant.isOpen ? 'bg-[#98E32F] animate-pulse' : 'bg-red-400'}`}></div>
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="relative">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setActiveDropdownId(activeDropdownId === restaurant._id ? null : restaurant._id);
                      }}
                ...
                <div className="flex gap-3">
                  <button 
                    onClick={() => handleEdit(restaurant)}
                    className="flex-1 bg-white/5 border border-white/10 py-2.5 rounded-xl text-sm font-medium hover:bg-white/10 transition-colors flex items-center justify-center gap-2"
                  >
                    Edit Details
                  </button>
                  ...
                  <div className="space-y-8 sm:space-y-10">
                    <section>
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-[#98E32F]">Basic Information</h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                          <label className="text-xs font-bold text-white/50 mb-2 block ml-1 uppercase tracking-wider">Brand Logo</label>
                          <div className="relative">
                            <input 
                              type="file" 
                              id="logo-upload"
                            ...
                            <label 
                              ...
                              className={`w-full h-[58px] bg-white/5 border border-dashed rounded-2xl flex items-center justify-center gap-2 cursor-pointer transition-all overflow-hidden relative group ${
                                logoUrl ? 'border-[#98E32F]/50 bg-[#98E32F]/5' : 'border-white/10 text-white/30 hover:bg-[#98E32F]/5 hover:text-[#98E32F] hover:border-[#98E32F]/50'
                              }`}
                            >
                              {(() => {
                                if (uploadingFile === 'logo') {
                                  return <Loader2 className="animate-spin text-[#98E32F]" size={18} />;
                        ...
                        <div className="md:col-span-3">
                          <label className="text-xs font-bold text-white/50 mb-2 block ml-1 uppercase tracking-wider">Description</label>
                          <textarea 
                        ...
                        <div className="relative group">
                          <label className="text-xs font-bold text-white/50 mb-2 block ml-1 uppercase tracking-wider">Support Email</label>
                          <div className="relative">
                            <Mail className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.email ? 'text-red-500' : 'text-white/20 group-focus-within:text-[#98E32F]'}`} size={18} />
                            <input 
                              {...register('email')}
                              type="email" 
                        ...
                        <div className="relative group">
                          <label className="text-xs font-bold text-white/50 mb-2 block ml-1 uppercase tracking-wider">Phone Number</label>
                          <div className="relative">
                            <Phone className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.contactNumber ? 'text-red-500' : 'text-white/20 group-focus-within:text-[#98E32F]'}`} size={18} />
                            <input 
                          ...
                          {errors.contactNumber && <p className="text-red-500 text-[10px] mt-1 ml-1 font-bold italic">{errors.contactNumber.message}</p>}
                        </div>
                      </div>
                    </section>
                    ...
                    <section>
                      <div className="flex items-center gap-3 mb-6">
                        <div className="h-px flex-1 bg-gradient-to-r from-transparent to-white/10"></div>
                        <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-[#98E32F]">Location & Address</h4>
                        <div className="h-px flex-1 bg-gradient-to-l from-transparent to-white/10"></div>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="md:col-span-3">
                          <label className="text-xs font-bold text-white/50 mb-2 block ml-1 uppercase tracking-wider">Street Address</label>
                          <div className="relative group">
                            <MapPin className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${errors.address?.street ? 'text-red-500' : 'text-white/20 group-focus-within:text-[#98E32F]'}`} size={18} />
                            <input 
                          ...
                          {errors.address?.street && <p className="text-red-500 text-[10px] mt-1 ml-1 font-bold italic">{errors.address.street.message}</p>}
                        </div>
                        <div>
                          <input 
                            ...
                            placeholder="City" 
                          />
                          {errors.address?.city && <p className="text-red-500 text-[10px] mt-1 ml-1 font-bold italic">{errors.address.city.message}</p>}
                        </div>
                        <div>
                          <input 
                        ...
                        <div>
                          ...
                          />
                          {errors.address?.zipCode && <p className="text-red-500 text-[10px] mt-1 ml-1 font-bold italic">{errors.address.zipCode.message}</p>}
                        </div>
                      </div>
                    </section>
                    ...
                    <section className="bg-[#98E32F]/[0.02] border border-[#98E32F]/10 rounded-[2rem] p-8">
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-[#98E32F]">Compliance & Documents</h4>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <label className="text-[10px] font-bold text-white/30 mb-2 block uppercase text-red-400">FSSAI License Number *</label>
                          <input 
                          ...
                          />
                          {errors.legalDocs?.fssaiLicenseNumber && <p className="text-red-500 text-[10px] mt-1 ml-1 font-bold italic">{errors.legalDocs.fssaiLicenseNumber.message}</p>}
                        </div>
                        <div>
                          <label className="text-[10px] font-bold text-white/30 mb-2 block uppercase">Business PAN</label>
                          <input 
                        ...
                        <div>
                          <label className="text-[10px] font-bold text-white/30 mb-2 block uppercase">GST Number (Optional)</label>
                          <input 
                        ...
                        <div>
                          <label className="text-[10px] font-bold text-white/30 mb-2 block uppercase">Trade License / D&O No.</label>
                          <input 
                        ...
                        <div className="md:col-span-2 mt-4">
                          <div className="flex items-center gap-3 mb-4">
                            <label className="text-[10px] font-bold text-white/30 block uppercase tracking-widest">Upload Digital Copies</label>
                            <div className="h-px flex-1 bg-white/5"></div>
                          </div>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {[
                              { label: 'FSSAI Certificate', key: 'fssaiCertificate' },
                              { label: 'GST Certificate', key: 'gstCertificate' },
                              { label: 'Trade License', key: 'tradeLicense' },
                              { label: 'Health Certificate', key: 'healthCertificate' },
                            ].map((doc) => (
                              <div key={doc.key} className="relative group">
                                <input 
                                  ...
                                  className="hidden" 
                                  accept=".pdf,image/*"
                                  onChange={(e) => handleFileUpload(e, doc.key, 'documents')}
                                />
                                <label 
                                  ...
                                  className={`w-full aspect-[4/3] bg-[#013644] border border-white/10 border-dashed rounded-[1.5rem] flex flex-col items-center justify-center gap-3 cursor-pointer transition-all p-4 group-active:scale-95 ${
                                    docsUrls[doc.key] ? 'border-[#98E32F]/40 bg-[#98E32F]/5' : 'hover:bg-[#98E32F]/5 hover:border-[#98E32F]/40'
                                  }`}
                                >
                                  <div className={`p-3 bg-white/5 rounded-full transition-all ${
                                    docsUrls[doc.key] ? 'text-[#98E32F] bg-[#98E32F]/10' : 'text-white/20 group-hover:text-[#98E32F] group-hover:bg-[#98E32F]/10'
                                  }`}>
                                  ...
                                  <div className="text-center">
                                    <p className={`text-[10px] font-bold transition-colors ${
                                      docsUrls[doc.key] ? 'text-white' : 'text-white/40 group-hover:text-white'
                                    }`}>{doc.label}</p>
                                    <p className="text-[8px] text-white/20 mt-1 uppercase tracking-tighter">
                                      {docsUrls[doc.key] ? 'Uploaded' : 'PDF or Image'}
                                    </p>
                                  </div>
                                  {docsUrls[doc.key] && (
                                    <button
                                      onClick={(e) => handleRemoveFile(e, doc.key)}
